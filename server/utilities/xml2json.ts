...
export async function xmlToJson(xml) {
  ...
  function getJSON(xmlparsed) {
    ...
    if (xmlparsed.attributes.length) q["attributes"] = getAttributes(xmlparsed);
  ...
  // Setting declared attributes
  if (doc.xmlVersion || doc.xmlEncoding) json["attributes"] = {};
  if (doc.xmlVersion) json["attributes"]["version"] = doc.xmlVersion;
  if (doc.xmlEncoding) json["attributes"]["encoding"] = doc.xmlEncoding;
